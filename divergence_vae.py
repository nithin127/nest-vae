...
parser.add_argument('--divergence', type=str, default='kl_divergence',
                    help='Type of divergence to be used (in [kl_divergence, jensen, hellinger, wasserstein], '
...
while steps < args.num_steps:
    for images, _ in data_loader:
        ...
        divergence_loss = 0
        for lol in range(len(images)):
            logvar_vec = log_var[lol,:]
            logvar2_vec = 1/logvar_vec
            var = torch.exp(logvar_vec)
            oneplus_var = 1 + var
            inv_oneplus_var = 1/oneplus_var
            mean_vec = mu[lol,:]
            kl_divergence = torch.sum(0.5 * (mean_vec ** 2 + torch.exp(logvar_vec) - logvar_vec - 1))
            jensen = torch.sum(0.25 * (-2 + torch.exp(logvar_vec) + torch.exp(-logvar_vec) + mean_vec *mean_vec*logvar2_vec + mean_vec ** 2 ))
            hellinger = 1 - torch.pow(torch.sum(var) , 0.25) * torch.pow( (torch.sum(oneplus_var))/2, -0.5) * torch.exp(-0.25*( torch.sum( mean_vec *  inv_oneplus_var * mean_vec)))
            wasserstein = torch.sum(mean_vec ** 2 + 1 + var - 2 * torch.pow(var, 0.5) )
            if args.divergence == 'kl_divergence':
                divergence_loss = divergence_loss + kl_divergence
            elif args.divergence == 'jensen':
                divergence_loss = divergence_loss + jensen
            elif args.divergence == 'hellinger':
                divergence_loss = divergence_loss + hellinger*10000
            elif args.divergence == 'wasserstein':
                divergence_loss = divergence_loss + wasserstein*100
            else:
                print('you have to specify the damn divergence you want to use')

        divergence_loss = divergence_loss/len(images)
        divergence_loss = args.beta * divergence_loss

        if args.C is not None:
            C = min(args.C * float(steps) / 100000, args.C)
            gamma = 1000
            loss = reconst_loss + gamma * torch.abs(divergence_loss - C)
            writer.add_scalar('C', C, steps)
        else:
            loss = reconst_loss + divergence_loss
        ...
        writer.add_scalar('jensen', jensen.data[0], steps)
        writer.add_scalar('hellinger', hellinger.data[0], steps)
        writer.add_scalar('wasserstein', wasserstein.data[0], steps)
