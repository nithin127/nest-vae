...
for beta_epoch in range(50):
    ...
    for epoch in range(10):
        for i, (images, _) in enumerate(data_loader):
            ...
            kl_divergence_ = 0.5 * (mu ** 2 + torch.exp(log_var) - log_var - 1)
            kl_divergence = torch.sum(torch.matmul(kl_divergence_, beta.unsqueeze(1)))
            ...
            if i % 100 == 0:
                print ("Epoch[%d/%d], Step [%d/%d], Total Loss: %.4f, "
                       ...
                       %(epoch + 1, 10, i + 1, iter_per_epoch, total_loss.data[0],
        ...
        writer.add_image('vae/reconstruction', reconst_grid, beta_epoch * 10 + epoch)
        ...
        torch.save(state, './.saves/{0}/{0}_{1:d}_{2:d}.ckpt'.format(output_folder, beta_epoch + 1, epoch + 1))
